"""
Re-annotate PersuasionForGood persuader turns using Phi-4 (14B, Microsoft)
...
def annotate_turn(utterance, context, session, retries=2):
    """Send one utterance to Phi-4 and parse the response."""
...
def main():
    ...
    log(f"Starting Phi-4 annotation")
    ...
    valid = result_df["phi4_strategy"].ne("").sum()
