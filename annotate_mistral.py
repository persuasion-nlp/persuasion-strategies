"""
Re-annotate PersuasionForGood persuader turns using Mistral-Small-3.2
via Ollama, replicating the Qwen3:30b methodology.
...
OUTPUT_CSV = os.path.join(os.path.dirname(__file__),
    "mistral_annotations.csv")
CHECKPOINT_CSV = os.path.join(os.path.dirname(__file__),
    "mistral_checkpoint.csv")
LOG_FILE = os.path.join(os.path.dirname(__file__),
    "mistral_annotation.log")
...
def annotate_turn(utterance, context, session, retries=2):
    """Send one utterance to Mistral-Small-3.2 and parse the response."""
...
def main():
    ...
    log(f"Starting Mistral-Small-3.2 annotation")
    ...
    for i, idx in enumerate(persuader_indices):
        ...
        results.append({
            ...
            "turn": row["Turn"],
            "utterance": utterance,
            "mistral_category": cat,
            "mistral_strategy": strat,
            "qwen_strategy": row.get("strategy_ollama_single", "")
        })
    ...
    valid = result_df["mistral_strategy"].ne("").sum()
